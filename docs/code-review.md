...
- Overview-page props and today task navigation were moved into `src/features/dashboard/model/useOverviewController.ts`; `useDashboardData` no longer owns click callbacks for today tasks.
- Member group assignment and member deletion orchestration were moved into `src/features/dashboard/model/usePeopleActionsController.ts`.

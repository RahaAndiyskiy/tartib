...
| `notifications` | partial | pending | partial | pending | partial | Notification modal UI and mark-read action are in `src/modules/notifications`; dashboard still owns modal open state and payment action callbacks. External push UI is postponed. |
...
- `src/features/dashboard/components/PaymentWorkspaceSection.tsx` owns the dashboard composition of the owner/trainer payment registry and drawer.
...
- `src/features/dashboard/model/useExpensesController.ts` owns expenses draft state, create expense and mark-paid handlers.
...
- `src/features/dashboard/model/useAccountRuntime.ts` owns local reset, new window and sign-out actions; external push methods are inert while push is postponed.
